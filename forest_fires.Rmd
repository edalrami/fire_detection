...
```
 Once removed we can finally see the results of our model, where all p-values are considerably low. Therefore the best features for our model include `month`, `FFMC`, `temp`, `RH`, and `wind`. Our model has a r-squared value of 0.66, inidcating that our model would likely be moderately useful in predicting the initial rate of spread of a fire.   
